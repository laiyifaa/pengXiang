<template>
  <el-dialog
    ...
    <el-form-item label="入学日期" prop="admissionDate">
      <el-input v-model="dataForm.admissionDate" placeholder="入学日期"></el-input>
    </el-form-item>
    <el-form-item label="离校日期" prop="leaveDate">
      <el-input v-model="dataForm.leaveDate" placeholder="离校日期"></el-input>
    </el-form-item>
    <el-form-item label="离校原因(上岗就业或自主择业)" prop="leaveReason">
    ...
    <el-form-item label="试用期限" prop="trialPeriod">
      <el-input v-model="dataForm.trialPeriod" placeholder="试用期限"></el-input>
    </el-form-item>
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/generator/inememployedsturecord/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.stuId = data.inemEmployedStuRecord.stuId
                this.dataForm.deptId = data.inemEmployedStuRecord.deptId
