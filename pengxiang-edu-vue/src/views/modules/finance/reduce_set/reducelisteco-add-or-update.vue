  ...
  <el-dialog
    ...
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()"
      ...
      <el-form-item label="家庭困难类型" prop="typeName">
        <el-input v-model="dataForm.typeName" placeholder="助学金类型"></el-input>
      </el-form-item>
...
export default {
  data () {
    return {
      ...
      dataRule: {
        typeName: [
          {required: true, message: '家庭困难类型不能为空', trigger: 'blur'}
        ],
        reduceTrainFee: [
  ...
  methods: {
    ...
    dataFormSubmit () {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          this.$http({
            url: this.$http.adornUrl(`/generator/reducelisteco/${!this.dataForm.id ? 'save' : 'update'}`),
            method: 'post',
