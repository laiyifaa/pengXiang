...
@Slf4j
public class StuBaseInfoListener implements ReadListener<StuBaseInfoEntity> {
    /**
     * 每隔5条存储数据库，实际使用中可以100条，然后清理list ，方便内存回收
     */
    private static final int BATCH_COUNT = 1024;
    private List<StuBaseInfoEntity> cachedDataList = ListUtils.newArrayListWithExpectedSize(BATCH_COUNT);

    private StuBaseInfoService stuBaseInfoService;

    public StuBaseInfoListener(StuBaseInfoService stuBaseInfoService) {
        this.stuBaseInfoService = stuBaseInfoService;
    }

    public StuBaseInfoListener() {
     ...
     */
    @Override
    public void invoke(StuBaseInfoEntity data, AnalysisContext context) {
    ...
    private void saveData() {
        ...
        stuBaseInfoService.importByList(cachedDataList);

    }
    private void check(StuBaseInfoEntity data, AnalysisContext context){
        ...
        if(StringUtils.isEmpty(data.getClassName())){
            throw new EmptyDataException("班级字段为空，第 " + (context.readRowHolder().getRowIndex() + 1) + " 行数据异常！");
        }
    }
}