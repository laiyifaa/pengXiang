...
@Slf4j
public class StuPracticeListener extends AnalysisEventListener<StuPracticeEntity> {
    /**
     * 每隔5条存储数据库，实际使用中可以100条，然后清理list ，方便内存回收
     */
    private static final int BATCH_COUNT = 1024;
    private List<StuPracticeEntity> cachedDataList = ListUtils.newArrayListWithExpectedSize(BATCH_COUNT);

    private WorkService workService;

    public StuPracticeListener(WorkService workService) {
     ...
     */
    @Override
    public void invoke(StuPracticeEntity data, AnalysisContext context) {
    ...
    private void saveData() {
        ...
        workService.importByList(cachedDataList);
    }
